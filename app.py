...
@st.cache_data
def load_data():
    # ── Try local CSV first (local dev) ──────────────────────────────────────
    local = p("Sample_200k.csv")
    if os.path.exists(local):
        df = pd.read_csv(local)
    else:
        # ── Fetch from LAPD Open Data API (Streamlit Cloud) ──────────────────
        import requests, io
        url = "https://data.lacity.org/resource/2nrs-mtv8.csv?$limit=50000&$order=date_occ+DESC"
        with st.spinner("📡 Fetching crime data from LAPD Open Data…"):
            resp = requests.get(url, timeout=60)
            resp.raise_for_status()
            df = pd.read_csv(io.StringIO(resp.text))
        # Normalise column names to match local CSV style
        df.columns = [c.upper().replace("_"," ") for c in df.columns]
        # Ensure required columns exist with fallbacks
        for col, default in [("WEAPON DESC", "No Weapon"), ("VICT DESCENT", "Unknown"),
                              ("WEAPON USED CD", 0), ("PART 1-2", 2)]:
            if col not in df.columns:
                df[col] = default
        # Rename to expected names
        rename = {
            "DR NO": "DR_NO", "DATE RPTD": "Date Rptd", "DATE OCC": "DATE OCC",
            "TIME OCC": "TIME OCC", "AREA NAME": "AREA NAME", "CRM CD DESC": "Crm Cd Desc",
            "VICT AGE": "Vict Age", "VICT SEX": "Vict Sex", "VICT DESCENT": "Vict Descent",
            "PREMIS CD": "Premis Cd", "WEAPON USED CD": "Weapon Used Cd",
            "WEAPON DESC": "Weapon Desc", "STATUS DESC": "Status Desc",
            "PART 1-2": "Part 1-2", "LAT": "LAT", "LON": "LON", "CRM CD 1": "Crm Cd 1",
        }
...
def load_models():
    files = {
        "LogisticRegression":    p("Files/LogisticRegression.pkl"),
        "RandomForest":          p("Files/RandomForestClassifier.pkl"),
        "XGBoost":               p("Files/XGBoost.pkl"),
        "LightGBM":              p("Files/LightGBM.pkl"),
        "CatBoost":              p("Files/CatBoost.pkl"),
        "DecisionTree":          p("Files/DecisionTreeClassifier.pkl"),
        "ExtraTrees":            p("Files/ExtraTreesClassifier.pkl"),
        "KNN":                   p("Files/KNeighborsClassifier.pkl"),
        "GaussianNB":            p("Files/GaussianNB.pkl"),
        "Bagging":               p("Files/Bagging.pkl"),
        "AdaBoost":              p("Files/AdaBoost.pkl"),
    }
    loaded = {}
...
def load_preprocessors():
    preps = {}
    paths = {
        "le_y":         p("Files/le_y.pkl"),
        "yeo":          p("Files/yeo-johnson.pkl"),
        "std":          p("Files/StandardScaler.pkl"),
        "std2":         p("Files/StandardScaler2.pkl"),
        "robust":       p("Files/RobustScaler.pkl"),
        "le_crm":       p("Files/Crm Cd Desc_label_encoder.pkl"),
        "le_descent":   p("Files/Vict Descent_label_encoder.pkl"),
        "le_weapon":    p("Files/Weapon Desc_label_encoder.pkl"),
        "le_status":    p("Files/Status Desc_label_encoder.pkl"),
        "imputer":      p("Files/Iterative_imputer_Age.pkl"),
    }
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        with open(p("Files/results_dict.pkl"), "rb") as f:
    ...
    with col2:
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            line_color=RED, fillcolor="rgba(233,69,96,0.15)"
        ))
        fig.add_trace(go.Scatterpolar(
            ...
            line_color=BLUE, fillcolor="rgba(59,130,246,0.15)"
        ))
        fig.update_layout(
            polar=dict(
